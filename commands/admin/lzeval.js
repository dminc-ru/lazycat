    ...
    async run (client, message, args) {
		...
		return result.then((output) => {
			...
			if (output.length > (1024 - (message.content.length))) {
                let completed = client.utils.success(`Код выполнен. Результат отправлен в консоль.`)
			...
			if (err.length > (1024 - (message.content.length))) {
                let completed = client.utils.success(`Произошла ошибка. Результат отправлен в консоль.`)
                message.channel.send({ embeds: [completed] })
				client.logger.log(`Evaluated code via /lzeval`, 'debug')
